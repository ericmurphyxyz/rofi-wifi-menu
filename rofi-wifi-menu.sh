...
if [ "$chosen_network" = "" ]; then
	...
	if [[ $check_saved_connections = "$chosen_id" ]]; then
		nmcli connection up id "$chosen_id" | grep "successfully" || wifi_password=$(rofi -dmenu -p "Password: " ) && nmcli device wifi connect "$chosen_id" password "$wifi_password" | grep "successfully" && notify-send "Connection Established" "$success_message"
	else
		if [[ "$chosen_network" =~ "" ]]; then
